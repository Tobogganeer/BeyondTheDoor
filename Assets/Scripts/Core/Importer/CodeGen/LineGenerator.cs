...
namespace BeyondTheDoor.Importer.CodeGen
{
    public static class LineGenerator
    {
        ...
        */
        ...
        /// <exception cref="ArgumentNullException"></exception>
        /// <exception cref="ArgumentException"></exception>
        public static void GenerateLinesFile(RawLineCollection rawLines)
        {
            if (rawLines == null)
        ...
        static void GenerateLineCode(RawLineData line, CodeFile cf)
        {
        ...
        static void GenerateAllLinesDict(RawLineCollection rawLines, CodeFile cf)
        {
            GenerateLineDict(cf, "All", rawLines.RawLines, "All lines that can be used.");
        }

        static void GenerateLineDict(CodeFile cf, string dictName, List<RawLineData> lines, string xmlComment = null)
        {
            System.Text.StringBuilder sb = cf.GetStringBuilder();
        ...
        /// </summary>
        public static void ClearLinesFile(RawLineCollection lines)
        {
            CodeFile cf = new CodeFile(nameof(Line) + ".Autogenerated", FilePaths.Autogenerated);
            cf.AddAutoGeneratedPreamble();
            ...
            using (cf.Namespace(FilePaths.DialogueNamespace))
            {
                using (cf.Class(CodeFile.Modifiers.Public | CodeFile.Modifiers.Partial, nameof(Line)))
                {
                    StringBuilder sb = cf.GetStringBuilder();

                    // Add null lines so scripts referencing these won't complain
                    foreach (var line in lines.RawLines)
                    {
                        cf.ApplyIndent();
                        sb.Append("public static Line ").Append(line.id).AppendLine(" { get; private set; } = null;");
                    }

                    cf.Space();
                    cf.ApplyIndent();
                    sb.AppendLine("public static Dictionary<LineID, Line> All { get; private set; }");
                }
            }
